...
public class ImageLoaders{
    ...
    private static ImageLoaders mInstance = null;
    ...
    private DisplayImageOptions getDefaultOptions() {
        DisplayImageOptions options = new DisplayImageOptions.Builder()
                //图片地址为空的时候
                .showImageForEmptyUri(R.drawable.ela_logo)
                //图片下载失败的时候显示
                .showImageOnFail(R.drawable.ela_logo)
                //图片可以缓存在内存
                .cacheInMemory(true)
                //图片可以缓存在硬盘
                .cacheOnDisk(true)
    ...
    public static ImageLoaders getInstance(Context context) {
        if (mInstance == null) {
            synchronized (ImageLoaders.class) {
                if (mInstance == null) {
                    mInstance = new ImageLoaders(context);
                }
            }
        }
        return mInstance;
